    ...
    void unionSet(string x, string y, double r){ //x = y *ｒ
        ...
        if (get<0>(rootX) != get<0>(rootY)){
            if (rank[get<0>(rootX)] >= rank[get<0>(rootY)]){
                ...
                get<1>(root[get<0>(rootY)]) = get<1>(rootX) / get<1>(rootY) * r; //(the ratio from y to x's root) = (ratio from x to x's root) / (ratio from y to y's root) * r
                rank[get<0>(rootX)] += rank[get<0>(rootY)];
            }
            else{
                get<0>(root[get<0>(rootX)]) = get<0>(rootY); //x's new root is y's root
                get<1>(root[get<0>(rootX)]) = get<1>(rootY) / get<1>(rootX) / r; //(the ratio from x to y's root) = (ratio from y to y's root) / (ratio from x to x's root) / r
...
public:
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries){
        ...
        //store data
        int numberOfEQs = equations.size();
        for(int i = 0; i < numberOfEQs; i++){
            string var_0 = equations[i][0];
            string var_1 = equations[i][1];
            double ratio = values[i];

            UF->unionSet(var_0, var_1, ratio);
        }
        ...
        for(int i = 0; i < queries.size(); i++){
            ...
            if(UF->isValid(var_0) == false or UF->isValid(var_1) == false){
                results.push_back(-1);
                continue;
            }
